#!/usr/bin/env python3
"""
MLX-LM Server Wrapper

This script manages MLX-LM server instances:
...
# Configuration
DEFAULT_AUTOCOMPLETE_MODEL = "mlx-community/Qwen1.5-1.8B-Chat-8bit"
DEFAULT_AUTOCOMPLETE_PORT = 11434
DEFAULT_DYNAMIC_PORT = 11433
DEFAULT_HOST = "127.0.0.1"

def start_mlx_server(model_name: str, port: int, max_tokens: int = 8192) -> subprocess.Popen:
    """Start an MLX-LM server for the specified model."""
    logger.info(f"Starting MLX-LM server for model {model_name} on port {port}")
    ...
    cmd = [
        ...
        "--log-file", f"mlx_server_{base_model_name}_{port}.log"
    ]
    ...
    while retries < max_retries:
        try:
            ...
            if response.status_code == 200:
                logger.info(f"MLX-LM server for {base_model_name} on port {port} is ready!")
                return process
        ...
        if process.poll() is not None:
            ...
            logger.error(f"Failed to start MLX-LM server: {stderr}")
            raise RuntimeError(f"Failed to start MLX-LM server for {model_name}: {stderr}")
    
    # If we get here, the server didn't start in time
    logger.error(f"Timed out waiting for MLX-LM server on port {port} to start")
    kill_process(process)
    raise TimeoutError(f"Timed out waiting for MLX-LM server for {model_name} to start")
...
def switch_model(new_model: str) -> Tuple[bool, str]:
    ...
    with model_lock:
        ...
        if current_model == new_model and model_process and model_process.poll() is None:
...
def status():
    """Return the status of the MLX-LM servers."""
    autocomplete_status = "running" if (
        autocomplete_process and autocomplete_process.poll() is None
    ) else "stopped"
    
    dynamic_status = "running" if (
        model_process and model_process.poll() is None
    ) else "stopped"
...
def start_servers():
    ...
    try:
        autocomplete_process = start_mlx_server(
            ...
            max_tokens=1024  # Lower token count for autocomplete model
        )
        logger.info(f"Autocomplete model server started on port {args.autocomplete_port}")
    except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MLX-LM Server Wrapper")
    parser.add_argument("--autocomplete-model", default=DEFAULT_AUTOCOMPLETE_MODEL,
                      help=f"Autocomplete model to use (default: {DEFAULT_AUTOCOMPLETE_MODEL})")
    parser.add_argument("--autocomplete-port", type=int, default=DEFAULT_AUTOCOMPLETE_PORT,
                      help=f"Port for autocomplete model server (default: {DEFAULT_AUTOCOMPLETE_PORT})")
    parser.add_argument("--dynamic-port", type=int, default=DEFAULT_DYNAMIC_PORT,
                      help=f"Port for dynamic model server (default: {DEFAULT_DYNAMIC_PORT})")
    parser.add_argument("--management-port", type=int, default=11435,
                      help="Port for management API (default: 11435)")
    ...
    # Start the servers
    if start_servers():
        logger.info(f"Started MLX-LM wrapper with autocomplete model: {args.autocomplete_model}")
        logger.info(f"Management API running on http://{args.host}:{args.management_port}")
        ...
        logger.error("Failed to start MLX-LM wrapper")
        cleanup()